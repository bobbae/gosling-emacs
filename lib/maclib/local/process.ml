...
(defun
	...
	(setq needs-checkpointing 0)
	(if (< (process-status "shell") 0)
	    (start-process "csh" "shell"))
	(local-bind-to-key "pr-newline-csh" '^m')
	(local-bind-to-key "send-eot" '')
	(local-bind-to-key "send-int-signal" '^?') ; /177 is losing!
	(local-bind-to-key "send-quit-signal" '^\')
	(local-bind-to-key "grab-last-line" (+ 128 '='))
	(end-of-file)
	(novalue)
...
(defun
    (new-c-shell name
	(setq name (arg 1 ": new-c-shell (buffer name) "))
	(pop-to-buffer name)
	(use-abbrev-table "shell")
	(use-syntax-table "shell")
	(setq abbrev-mode 1)
...
(defun (shell-man
	   (if (= (- (dot) (mark)) 3)
	       (progn entry
		      (setq entry (get-tty-string ": manual-entry (for) "))
		      (manual-entry entry)
		      (insert-string " " entry)
		      (send-int-signal)
		      0)
...
(save-excursion
    ...
    (define-hooked-local-abbrev "man" "man" "shell-man")
    (use-syntax-table "shell")

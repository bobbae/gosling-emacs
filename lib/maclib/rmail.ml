...
(defun
	...
	(if (! (is-bound read-mail-abbrevs))
		...
		(quietly-read-abbrev-file "/usr/jtk/emacs/RMailAbbrevs")
	    )
	)
...
(defun    
	...
	(use-abbrev-table "RMail")
	(filter-region "/usr/jtk/emacs/makemail")
	(beginning-of-file)
	(search-forward "hErE<!}")
...
(defun
	...
	(set-mark)
	(end-of-file)
	(filter-region (concat "/etc/delivermail -i -s -p "
			       addresses))
	(if (= (buffer-size) 0)
	    (progn
		  (setq mail-do-send 0)

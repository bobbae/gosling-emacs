...
static IgnoreModifiedDiskCopy;  /* (bcp, 8/84) Hack to "fix" problem
				   with emacs getting confused about
				   whether a file has actually been
				   modified on disk */
...
register char  *fn; {
	...
	st.st_mtime > bf_modtime && !IgnoreModifiedDiskCopy)
    {
...
InitFIO () {
	...
	DefIntVar ("ignore-modified-disk-copy", &IgnoreModifiedDiskCopy);
	IgnoreModifiedDiskCopy = 0; /* I think this is redundant */

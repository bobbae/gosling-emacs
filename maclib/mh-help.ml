...
"    r: reply to message sender		R: reply to all recipients\n"
"    f: forward a message		y: retry msg in saved buffer\n"
"    e: edit message			g: get a new folder\n"
"    b: get a bboard folder		\n"
"    q: check outgoing mail queue	^X^C: exit.\n"
...
"shell."
	    ...
	    (beginning-of-file)
	    (error-occured (if (< (window-height) 12)
			       (provide-prefix-argument (- 12 (window-height))
				   (enlarge-window))
			   )
	    )
